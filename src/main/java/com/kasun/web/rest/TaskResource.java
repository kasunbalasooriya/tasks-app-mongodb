...
import com.kasun.domain.Task;
import com.kasun.repository.TaskRepository;
...
/**
 * REST controller for managing {@link com.kasun.domain.Task}.
 */
@RestController
@RequestMapping("/api")
public class TaskResource {
     ...
     */
    @PutMapping("/tasks/{id}")
    public Mono<ResponseEntity<Task>> updateTask(@PathVariable(value = "id", required = false) final String id, @RequestBody Task task)
    ...
    public Mono<ResponseEntity<Task>> partialUpdateTask(
        ...
        return taskRepository
            ...
            .flatMap(exists -> {
                ...
                Mono<Task> result = taskRepository
                    .findById(task.getId())
                    .map(existingTask -> {
